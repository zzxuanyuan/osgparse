...
class FormattedLifecycle:
	...
	def dump(self):
		...
		print "desktop_time_info : ", sorted(self.desktop_time_info)
...
class LifecycleFormatter:
	...
	def labeling(self,job_freq_history_dict,job_time_history_dict):
		...
		elif max(self.lifecycle.activity_dict.iteritems(), key=operator.itemgetter(1))[0] is "Idle":
			if last_activity == "Idle":
				self.formatted_lifecycle.label = "CleanUp"
			else:
				self.formatted_lifecycle.label = last_activity+"Stupid"
		elif max(self.lifecycle.activity_dict.iteritems(), key=operator.itemgetter(1))[0] is "Busy":
			if last_activity == "Idle":
		...
		elif max(self.lifecycle.activity_dict.iteritems(), key=operator.itemgetter(1))[0] is "Benchmarking":
