...
class CustomizedMajorityVoteRegressor:
	def __init__(self, neighbor=5, attributes=['TotalJobNumber', 'DesktopStartDateMinute','DesktopEndDateMinute'], target='DesktopEndDateMinute'):
#		assert attributes == ['DesktopStartDateMinute','DesktopEndDateMinute']
	...
	def predict(self, df_train, df_test, cur_desktop_time):
		df_train = df_train[['TotalJobNumber', 'DesktopStartDateMinute','DesktopEndDateMinute']]
		print "df_train has ", len(df_train.index), " samples"
		if len(df_train.index) < 20:
			return -1
		df_test = df_test[['TotalJobNumber', 'DesktopStartDateMinute','DesktopEndDateMinute']]
		...
		for idx in df_test.index:
			df_closest = find_closest(df_train, neighbor, 
