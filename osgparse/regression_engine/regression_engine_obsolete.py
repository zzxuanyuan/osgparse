...
#				'Class']
		...
		if regressor == 'DecisionTree':
			...
			self.regressor = decisiontree_regressor.CustomizedDecisionTreeRegressor(self.attributes, self.target)
		elif regressor == 'RandomForest':
			self.target = 'Duration'
			self.attributes = ['MaxRetireTime', 'MaxKillTime', 'TotalJobNumber', 'ResourceJobNumber', 'DesktopStartDateMinute', 'NumberOfHost', 'JobStartTime', 'PreemptionFrequency', 'Duration']
			self.regressor = randomforest_regressor.CustomizedRandomForestRegressor(self.attributes, self.target)
		elif regressor == 'KNN':
			self.target = 'Duration'
			self.attributes = ['MaxRetireTime', 'MaxKillTime', 'TotalJobNumber', 'ResourceJobNumber', 'DesktopStartDateMinute', 'NumberOfHost', 'JobStartTime', 'PreemptionFrequency', 'Duration']
			self.regressor = knn_regressor.CustomizedKNNRegressor(self.attributes, self.target)
		elif regressor == 'LinearRegression':
			self.target = 'DesktopEndDateMinute'
			self.attributes = ['DesktopStartDateMinute', 'DesktopEndDateMinute', 'Duration']
			self.regressor = linear_regressor.CustomizedLinearRegressor(self.attributes, self.target)
		elif regressor == 'PreemptionRegression':
			self.target = 'DesktopEndDateMinute'
